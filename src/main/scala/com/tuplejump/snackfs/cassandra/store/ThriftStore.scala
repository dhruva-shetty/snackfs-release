...
import org.apache.cassandra.thrift.Cassandra.AsyncClient.insert_call
...
class ThriftStore(configuration: SnackFSConfiguration) extends FileSystemStore {
  ...
  private var clientPool: ThriftConnectionPool = CassandraCluster.getThriftConnectionPool(configuration)
    ...
    ksDefFuture onSuccess {
      case p =>
        ...
        if (mayBeKsDef.isSuccess) {
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " Using existing keyspace %s", ksDef.getName)
          prom success new Keyspace(ksDef.getName)

        } else {
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " Creating new keyspace %s", ksDef.getName)
          val response = AsyncUtil.executeAsync[system_add_keyspace_call](
            client.system_add_keyspace(ksDef, _))

          response.onSuccess {
            case r =>
              if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " Created keyspace %s", ksDef.getName)
  ...
  def init {
    ...
    if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " initializing thrift store with configuration %s", configuration.toString)
    fsPermissionChecker = new FSPermissionChecker(this, user, configuration.useACL)
  }

  def dropKeyspace: Future[Unit] = clientPool.executeWithClient({
    client =>
      ...
      dropFuture onSuccess {
        case p =>
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " deleted keyspace %s", configuration.keySpace)
  ...
  def storeINode(path: Path, iNode: INode): Future[GenericOpSuccess] = clientPool.executeWithClient({
    client =>
      ...
      iNodeFuture.onSuccess {
        case p =>
          log.info(Thread.currentThread.getName() + " stored INode %s", iNode.toString)
  ...
  private def performGet(client: AsyncClient, key: ByteBuffer, columnPath: ColumnPath): Future[ColumnOrSuperColumn] = {
    ...
    getFuture.onSuccess {
      case p =>
        try {
          val res = p.getResult
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " fetch INode/subblock data")
          prom success res
        } catch {
          case e: Exception =>
  ...
  def retrieveINode(path: Path): Future[INode] = clientPool.executeWithClient({
    client =>
      ...
      if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " fetching Inode for path %s", path)
      val pathInfo = performGet(client, pathKey, inodeDataPath)

      pathInfo.onSuccess {
        case p =>
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " retrieved Inode for path %s", path)
  ...
  def storeSubBlock(blockId: UUID, subBlockMeta: SubBlockMeta, dataBuffer: ByteBuffer, compressed: Boolean): Future[GenericOpSuccess] = clientPool.executeWithClient({
    client =>
      ...
      val column = new Column()
        ...
        .setValue(ByteBuffer.wrap(LZ4CompressorInstance.compress(bytes, compressed)))
  ...
  def retrieveSubBlock(blockId: UUID, subBlockId: UUID, compressed: Boolean): Future[InputStream] = clientPool.executeWithClient({
    client =>
      ...
      subBlockFuture.onSuccess {
        case p =>
          ...
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " retrieved subBlock with id %s and block id %s", subBlockId.toString, blockId.toString)
          prom success stream
      }

      subBlockFuture.onFailure {
        case f =>
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " failed to retrieve subBlock with id %s and block id %s", subBlockId.toString, blockId.toString)
          prom failure f
      }
  ...
  def retrieveBlock(blockMeta: BlockMeta, compressed: Boolean, isLocalBlock: Boolean): InputStream = {
    if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " retrieve Block %s", blockMeta.toString)
  ...
  def deleteINode(path: Path): Future[GenericOpSuccess] = clientPool.executeWithClient({
    client =>
  ...
  def deleteBlocks(iNode: INode): Future[GenericOpSuccess] = clientPool.executeWithClient({
    client =>
      ...
      deleteFuture.onSuccess {
        case p =>
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " deleted blocks for INode %s", iNode.toString)
          result success GenericOpSuccess()
      }
  ...
  private def fetchPaths(indexExpr: List[IndexExpression]): Future[Set[Path]] = clientPool.executeWithClient({
    client =>
      ...
      val indexClause = new IndexClause(indexExpr, ByteBufferUtil.EMPTY_BYTE_BUFFER, 1000)
      ...
      if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " begin fetching subpaths for %s with index clause %s consistency level %s", indexExpr.toString(), indexClause.toString(), configuration.readConsistencyLevel.toString())
      ...
      rowFuture.onSuccess {
        case p =>
          ...
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " finished fetching subpaths for %s", indexExpr.toString())
 ...
 def getBlockLocations(path: Path): Future[Map[BlockMeta, List[String]]] = clientPool.getBlockLocations(path, retrieveINode(path))
  ...
  private def addLockColumn(path: Path, processId: UUID, client: AsyncClient): Future[insert_call] = {
    ...
    val column = new Column().setName(ByteBufferUtil.bytes(timeStamp)).setValue(ByteBufferUtil.bytes(processId)).setTimestamp(System.currentTimeMillis())
    val addColumnFuture = AsyncUtil.executeAsync[insert_call](client.insert(key, columnParent, column, configuration.writeConsistencyLevel, _))
    if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " adding column: " + key)
    addColumnFuture
  }
  ...
  private def getLockRow(path: Path, client: Cassandra.AsyncClient): Future[get_slice_call] = {
    ...
    val getRowFuture = AsyncUtil.executeAsync[get_slice_call](client.get_slice(key, columnParent, slicePredicate, configuration.writeConsistencyLevel, _))
    if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " getting row: " + key)
    getRowFuture
  }

  private def isCreator(processId: UUID, columns: java.util.List[ColumnOrSuperColumn]): Boolean = {
    var result = false
    if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " checking for access to create a file for %s", processId)
    if (columns.length >= 1) {
  ...
  def acquireFileLock(path: Path, processId: UUID): Future[Boolean] = clientPool.executeWithClient({
    client =>
      val prom = Promise[Boolean]()
      val addColumnFuture = addLockColumn(path, processId, client)
      addColumnFuture.onSuccess {
        case res =>
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " added column for create lock")
  ...
  private def deleteLockRow(path: Path, client: Cassandra.AsyncClient): Future[remove_call] = {
    ...
    val timestamp = System.currentTimeMillis
    val deleteLockFuture = AsyncUtil.executeAsync[remove_call](client.remove(getPathKey(path), columnPath, timestamp, configuration.writeConsistencyLevel, _))
    deleteLockFuture
  }

  def releaseFileLock(path: Path): Future[Boolean] = clientPool.executeWithClient({
    client =>
      val prom = Promise[Boolean]()
      val deleteLockFuture = deleteLockRow(path, client)
      deleteLockFuture.onSuccess {
        case res =>
          if(LogConfiguration.isDebugEnabled) log.debug(Thread.currentThread.getName() + " deleted lock")
          prom success true
      }
  ...
  def getRemoteAkkaClient: SnackFSAkkaClient = null
  
  def getRemoteNettyClient: SnackFSNettyClient = null
}
